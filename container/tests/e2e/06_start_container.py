...
Start terminail-mcp-server container from image
"""
import subprocess
import sys
import time
...
def start_container():
    """Start terminail-mcp-server container"""
    try:
        print("🚀 STEP 6: Start Container")
        print("   🔧 Starting terminail-mcp-server container...")
        ...
        if "terminail-mcp-server" in result.stdout:
            print("   ✅ terminail-mcp-server container is already running")
            return True
        ...
        if result.returncode == 0:
            ...
            if response.lower().strip() in ['y', 'yes']:
                ...
                if "terminail-mcp-server" in verify_result.stdout:
                    print("   ✅ terminail-mcp-server container is now running")
                    return True
                else:
                    print("   ❌ terminail-mcp-server container is still not running")
                    return False
            else:
...
def verify_container_running():
    ...
    try:
        ...
        result = subprocess.run(["podman", "ps"], capture_output=True, text=True)
        if "terminail-mcp-server" in result.stdout:
            print("   ✅ terminail-mcp-server container is running")
            return True
        else:
            print("   ❌ terminail-mcp-server container is not running")
            return False
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        print()
        response = input("   Is the terminail-mcp-server container running? (y/N): ")
        if response.lower().strip() in ['y', 'yes']:
