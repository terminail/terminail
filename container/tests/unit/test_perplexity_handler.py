"""
Unit tests for Perplexity AI handler
"""
import pytest
from unittest.mock import AsyncMock
from mcp_server.handlers.perplexity_handler import PerplexityHandler
from mcp_server.ai_handler_base import AIHandler


class TestPerplexityHandler:
    """Test cases for PerplexityHandler"""
    
    def test_handler_instantiation(self):
        """Test PerplexityHandler instantiation"""
        mock_page = AsyncMock()
        handler = PerplexityHandler(mock_page)
        
        assert handler is not None
        assert isinstance(handler, PerplexityHandler)
