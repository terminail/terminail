"""
Unit tests for Yuanbao AI handler
"""
import pytest
from unittest.mock import AsyncMock
from mcp_server.handlers.yuanbao_handler import YuanbaoHandler
from mcp_server.ai_handler_base import AIHandler


class TestYuanbaoHandler:
    """Test cases for YuanbaoHandler"""
    
    def test_handler_instantiation(self):
        """Test YuanbaoHandler instantiation"""
        mock_page = AsyncMock()
        handler = YuanbaoHandler(mock_page)
        
        assert handler is not None
        assert isinstance(handler, YuanbaoHandler)
