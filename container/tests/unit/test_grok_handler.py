"""
Unit tests for Grok AI handler
"""
import pytest
from unittest.mock import AsyncMock
from mcp_server.handlers.grok_handler import GrokHandler
from mcp_server.ai_handler_base import AIHandler


class TestGrokHandler:
    """Test cases for GrokHandler"""
    
    def test_handler_instantiation(self):
        """Test GrokHandler instantiation"""
        mock_page = AsyncMock()
        handler = GrokHandler(mock_page)
        
        assert handler is not None
        assert isinstance(handler, GrokHandler)
